...
namespace AssetStudio
{
    ...
    public static class FileReaderExtensions
    {
        public static FileReader PreProcessing(this FileReader reader, Game game)
        {
            ...
            {
                Logger.Info("File might have multiple bundles !!");
                try
                {
                    ...
                    {
                        Logger.Info($"Found signature {signature}, expected bundle size is 0x{size:X8}, found 0x{reader.BaseStream.Length} instead !!");
